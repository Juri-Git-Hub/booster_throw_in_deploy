...
class BeyondMimicPolicy(Policy):
    ...
    def reset(self) -> None:
        ...
        self.last_observation = torch.zeros(
            119, dtype=torch.float32, device=self.cfg.device)
        self.last_raw_action = torch.zeros(
            self.robot.num_joints,
            dtype=torch.float32, device=self.cfg.device)
        self.last_motion_frame = 0
        self.last_orientation_safety_dot = 1.0
    ...
    def compute_observation(self) -> torch.Tensor:
        ...
        self.last_observation = observation.flatten().detach().clone()
        return observation
    ...
    def inference(self) -> torch.Tensor:
        ...
        self.last_raw_action = action.detach().clone()
        self.last_motion_frame = self.current_frame
        ...
        if self.cfg.enable_safety_fallback:
            ...
            orientation_dot = torch.dot(
                projected_gravity, motion_projected_gravity
            )
            self.last_orientation_safety_dot = float(orientation_dot.item())
            if orientation_dot < 0.5:
...
class K1BeyondMimicControllerCfg(ControllerCfg):
    ...
    safety = SafetyCfg(
        ...
        joint_pos_lower=[
            -0.90, -0.289,
            -3.089, -1.575, -2.043, -2.318,
            -3.089, -1.405, -2.043, 0.122,
            -2.740, -0.302, -0.900, 0.112, -0.809, -0.311,
            -2.740, -1.472, -0.900, 0.112, -0.809, -0.311,
        ],
        joint_pos_upper=[
            0.90, 0.795,
            0.993, 1.405, 2.043, -0.122,
            0.993, 1.575, 2.043, 2.318,
            1.950, 1.472, 0.900, 2.118, 0.284, 0.311,
            1.950, 0.302, 0.900, 2.118, 0.284, 0.311,
        ],
    )
